 ...
 */
public class Pengu3 extends Mover3
{
    // "physics" variables
...
//             m.increaseLevel();
    ...
    public boolean onGround3()
    {
        ...
        {
            under = (Surface3)getOneObjectAtOffset ( 0, getImage().getHeight() / 2 + counter, Surface3.class);
            counter++;
        }
     ...
     */
    public Actor getSurface3()
    {
        Actor under = getOneObjectAtOffset ( 0, getImage().getHeight() / 2, Surface3.class);
        int counter = -1;
        int max = vSpeed + 2;
        while (counter <= max && under == null)
        {
            under = getOneObjectAtOffset ( 0, getImage().getHeight() / 2 + counter, Surface3.class);
            counter++;
        }
        return under;
    }
    ...
    public Enemy3 getEnemy3()
    {
        ...
        {
            Actor temp = getOneIntersectingObject(Enemy3.class);
            Enemy3 enemy3 = (Enemy3)temp;
            currCollDetection = 0;
            return enemy3;
        }
        else
    ...
    public boolean checkLeft()
    {
        ...
        {
            bumper = getOneObjectAtOffset (-1*( getImage().getWidth() / 2), max - counter, Surface2.class);
            counter++;
        }
        return bumper != null;
    }
    ...
    public boolean checkRight()
    {
        ...
        while (counter < max && bumper == null)
        {
            bumper = getOneObjectAtOffset ( getImage().getWidth() / 2, max - counter , Surface2.class);
            counter++;
        }
        return bumper != null;
    }
